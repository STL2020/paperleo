...
public class SettingsDto
{
    ...
    // Pro-Features: KI-Kostenkontrolle
    public int MaxPagesPerDocument { get; set; } = 0;         // 0 = unbegrenzt
    public decimal MonthlyCostLimitEur { get; set; } = 0;    // 0 = kein Limit
    public decimal DailyBudgetLimitEur { get; set; } = 0;    // 0 = kein Limit
    public int MonthlyTokenUsed { get; set; } = 0;            // wird automatisch gezählt
    public int MonthlyTokenLimit { get; set; } = 0;           // 0 = kein Limit
    public int ReasoningLevel { get; set; } = 0;              // 0=Standard, 1=Erweitert, 2=Maximum (Gemini/o-Modelle)
    ...
    public string? LogoKitApiKey { get; set; }  // LogoKit.com API Token für Korrespondenten-Logos
    public bool ProMode { get; set; }              // Wird server-seitig gesetzt, nicht persistiert

    // Titel-Builder Format
    public string? TitleParts     { get; set; }
    public string? TitleSeparator { get; set; }

    // Backup-Schedule
    public string? BackupSchedule    { get; set; }  // "none" | "daily" | "weekly" | "monthly"
    public string? BackupScheduleDay { get; set; }  // "monday".."sunday" für weekly
    public string? BackupTargetPath  { get; set; }  // lokaler Pfad im Container z.B. /app/data/backups
...
public class StatusDto
{
    ...
    public bool AutoModeEnabled { get; set; }
    public string? CurrentScanDocument { get; set; }
...
public class DashboardDto
{
    ...
    // System-Info (Host/Docker)
    public string? SystemUptime    { get; set; }
    public string? DotnetVersion   { get; set; }
    public string? OsDescription   { get; set; }
    public long    DiskUsedBytes   { get; set; }
    public long    DiskTotalBytes  { get; set; }
    public double  DiskUsedGb      => Math.Round(DiskUsedBytes  / 1_073_741_824.0, 1);
    public double  DiskTotalGb     => Math.Round(DiskTotalBytes / 1_073_741_824.0, 1);
    public int     DiskUsedPct     => DiskTotalBytes > 0 ? (int)(DiskUsedBytes * 100L / DiskTotalBytes) : 0;

    // KI-Kosten aktueller Monat
    public int     MonthTokensUsed     { get; set; }
    public int     MonthTokenLimit     { get; set; }
    public double  MonthCostEurEst     { get; set; }
    public int     MonthDocsProcessed  { get; set; }
}
...
/// <summary>
/// Dokument eines Korrespondenten für die Detailansicht
public class CorrespondentDocDto
{
    public int Id { get; set; }
    public string Title { get; set; } = "";
    public string? Created { get; set; }
...
/// Korrespondent mit Logo-Daten für die Web-Ansicht
...
public class ArchiveAnalysisRequest
{
...
public class ArchiveAnalysisResult
{
    ...
    public int DocumentsAnalyzed { get; set; }
    public List<ArchiveField> Fields { get; set; } = new();
}

public class ArchiveField
{
    public string Category { get; set; } = "";      // z.B. "Person", "Adresse", "Arbeitgeber"
    public string Key { get; set; } = "";            // z.B. "name", "steuernummer", "iban"
    public string Label { get; set; } = "";          // z.B. "Name", "Steuernummer", "IBAN"
    public string Value { get; set; } = "";          // gefundener Wert
    public int Confidence { get; set; }              // 0-100
    public int FoundInDocuments { get; set; }        // wie oft gefunden
    public string? ExampleDocument { get; set; }     // Dokumentname als Beleg
    public bool Accepted { get; set; } = false;      // vom User bestätigt
}

public record ArchiveAnalysisStatus
{
    public bool IsRunning { get; init; }
    public int Progress { get; init; }
    public string Phase { get; init; } = "";
    public ArchiveAnalysisResult? LastResult { get; init; }
}
...
public class BackupManifest
{
    ...
    public string PaperlessUrl       { get; set; } = "";
    public int TotalDocuments        { get; set; }
    public int DownloadedPdfs        { get; set; }
    ...
    public string PaperLeoVersion    { get; set; } = "";
}
...
public class BackupResult
{
    ...
    public List<int> FailedDocumentIds   { get; set; } = new();
}
...
public class RestoreResult
{
    ...
    public int CorrespondentsRestored { get; set; }
    public int DocumentTypesRestored  { get; set; }
    public int CustomFieldsRestored   { get; set; }
