...
public class PaperlessClient
{
    ...
    /// <summary>Gibt den konfigurierten HttpClient zurück — für Backup-Downloads.</summary>
    public HttpClient GetHttpClient() => _http;

    /// <summary>Lädt ein Dokument als PDF-Bytes herunter.</summary>
    public async Task<byte[]> DownloadDocumentAsync(int id, CancellationToken ct = default)
    {
        var resp = await SendWithRetryAsync(HttpMethod.Get, $"/api/documents/{id}/download/", null, ct);
    ...
    /// <summary>
    /// Lädt ein PDF-Dokument in Paperless-ngx hoch (POST /api/documents/post_document/).
    /// Paperless verarbeitet es asynchron — gibt Task-ID zurück.
    /// </summary>
        ...
        CancellationToken ct = default)
    {
        ...
        var resp = await _http.PostAsync("/api/documents/post_document/", form, ct);
        if (!resp.IsSuccessStatusCode) return null;
        var body = await resp.Content.ReadAsStringAsync(ct);
        return body.Trim('"');  // Paperless gibt Task-UUID zurück
    }
