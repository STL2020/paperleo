...
public class ApiClient(HttpClient http)
{
    ...
    /// </summary>
    public async Task<(bool isScanning, string? currentDoc)> GetScanStatusAsync()
    {
        try
        {
            var resp = await http.GetAsync("api/dashboard/scan-status");
            ...
            var scanning = doc.RootElement.GetProperty("isScanning").GetBoolean();
            var curDoc = doc.RootElement.GetProperty("currentDocument").GetString();
            return (scanning, curDoc);
        }
        catch { return (false, null); }
    }

    public async Task<bool> RestartAppAsync()
    {
        try { await http.PostAsync("api/dashboard/restart", null); return true; }
        catch { return true; } // Response may not arrive if restart is fast
    }

    public async Task<List<CorrespondentDocDto>> GetCorrespondentDocsAsync(int correspondentId)
    {
        var resp = await http.GetAsync($"api/dashboard/correspondent-docs/{correspondentId}");
        if (!resp.IsSuccessStatusCode) return new();
        return await resp.Content.ReadFromJsonAsync<List<CorrespondentDocDto>>() ?? new();
    }

    public async Task<List<CorrespondentDto>> GetCorrespondentsFullAsync()
    {
        var resp = await http.GetAsync("api/dashboard/correspondents-full");
        if (!resp.IsSuccessStatusCode) return new();
        return await resp.Content.ReadFromJsonAsync<List<CorrespondentDto>>() ?? new();
    }
    ...
    public async Task<ArchiveAnalysisStatus> GetArchiveAnalysisStatusAsync() =>
        await http.GetFromJsonAsync<ArchiveAnalysisStatus>("api/archive-analysis/status") ?? new();

    public async Task<bool> StartArchiveAnalysisAsync()
    {
        var resp = await http.PostAsync("api/archive-analysis/start", null);
        return resp.IsSuccessStatusCode;
    }

    public async Task<ArchiveAnalysisResult?> GetArchiveAnalysisResultAsync() =>
        await http.GetFromJsonAsync<ArchiveAnalysisResult>("api/archive-analysis/result");

    public async Task ClearArchiveAnalysisAsync() =>
        await http.DeleteAsync("api/archive-analysis/result");

    public async Task<string> ExportSettingsAsync()
    {
        var resp = await http.GetAsync("api/settings/export");
        return resp.IsSuccessStatusCode ? await resp.Content.ReadAsStringAsync() : "";
    }

    public async Task<(bool Success, string Message)> ImportSettingsAsync(string content)
    {
        var resp = await http.PostAsync("api/settings/import",
            new StringContent(content, System.Text.Encoding.UTF8, "text/plain"));
        var body = await resp.Content.ReadAsStringAsync();
        if (resp.IsSuccessStatusCode)
            return (true, "Einstellungen importiert ✓ — Seite wird neu geladen.");
        return (false, body.Length > 0 ? body : "Import fehlgeschlagen.");
    }
    ...
    public async Task<BackupStatusDto> GetBackupStatusAsync() =>
        await http.GetFromJsonAsync<BackupStatusDto>("api/backup/status") ?? new();

    public async Task<bool> StartBackupAsync()
    {
        var resp = await http.PostAsync("api/backup/start", null);
        return resp.IsSuccessStatusCode;
    }

    public string GetBackupDownloadUrl() => "api/backup/download";

    public async Task<RestoreResult> RestoreFromZipAsync(Stream zipStream)
    {
        var content = new StreamContent(zipStream);
        content.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/octet-stream");
        var resp = await http.PostAsync("api/backup/restore", content);
        return (await resp.Content.ReadFromJsonAsync<RestoreResult>()) ?? new();
    }

    public async Task ResetBackupStatusAsync() =>
        await http.DeleteAsync("api/backup/status");

    public async Task<RestoreResult> RestorePdfsFromZipAsync(Stream zipStream)
    {
        var content = new StreamContent(zipStream);
        content.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/octet-stream");
        var resp = await http.PostAsync("api/backup/restore-pdfs", content);
        return (await resp.Content.ReadFromJsonAsync<RestoreResult>()) ?? new();
    }
