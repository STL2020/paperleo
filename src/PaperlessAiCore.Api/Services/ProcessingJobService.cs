...
public class ProcessingJobService
{
    ...
    {
        // Mark stale queued/running jobs as failed on startup (from previous session)
        foreach (var job in _jobs.Values.Where(j => j.Status is "queued" or "running"))
        {
            job.Status = "failed";
            job.Error = "App-Neustart — Job wurde abgebrochen.";
