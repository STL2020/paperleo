    ...
    ProcessingJobService jobService) : IIngestScanService
{
    public async Task<int> RunScanAsync(SettingsDto settings, CancellationToken ct = default)
    {
        ...
        if (unprocessed.Count > 0) status.SetScanningStart();

        // Free/Community-Limit: max 50 Dokumente gesamt verarbeitet
        var isProMode = LicenseCheck.IsProMode(settings.PremiumLicenseKey);
        log.LogInformation("Lizenz-Check: ProMode={IsProMode}, Key={KeyStatus}",
            isProMode,
            string.IsNullOrWhiteSpace(settings.PremiumLicenseKey) ? "LEER/FEHLT" : $"gesetzt ({settings.PremiumLicenseKey.Length} Zeichen)");

        if (!isProMode)
        {
            var alreadyProcessed = await paperless.ListDocumentsAsync(
                new() { ["tags__id"] = processedTagId.ToString(), ["page_size"] = "1" }, ct);
            var totalProcessed = alreadyProcessed.Count;
            var remaining = Math.Max(0, PaperlessAiCore.Shared.AppInfo.FreeDocumentLimit - totalProcessed);
            if (remaining <= 0)
            {
                log.LogWarning("Community-Limit erreicht ({Limit} Dokumente). Bitte auf Pro upgraden.", PaperlessAiCore.Shared.AppInfo.FreeDocumentLimit);
                return 0;
            }
            unprocessed = unprocessed.Take(remaining).ToList();
            log.LogInformation("Community-Modus: {Remaining} von {Limit} verbleibende Slots.", remaining, PaperlessAiCore.Shared.AppInfo.FreeDocumentLimit);
        }

        // Pro: Tages-Budget-Limit prüfen
        if (settings.DailyBudgetLimitEur > 0)
        {
            // Einfache Schätzung: bisherige Monatskosten / Tage im Monat
            var todayEstimate = settings.MonthlyTokenUsed > 0
                ? (decimal)(settings.MonthlyTokenUsed / 1_000_000.0 * 0.30) / DateTime.UtcNow.Day
                : 0m;
            if (todayEstimate >= settings.DailyBudgetLimitEur)
            {
                log.LogWarning("Tages-Budgetlimit von €{Limit} erreicht (est. €{Est:F4} heute). Scan pausiert.",
                    settings.DailyBudgetLimitEur, todayEstimate);
                return 0;
            }
        }
        ...
        foreach (var doc in unprocessed)
        {
            ...
            status.SetCurrentDocument(docTitle); // Dashboard zeigt aktuelles Dokument
        ...
        foreach (var doc in unprocessed)
        {
            // Graceful cancel between documents (not mid-document)
            if (ct.IsCancellationRequested)
            {
                log.LogInformation("Scan abgebrochen nach {Count} Dokumenten (Token cancelled).", processedCount);
                break;
            }
            ...
            try
            {
                // Per-Dokument Timeout: max 3 Minuten — verhindert blockierte Queue
                using var docCts = CancellationTokenSource.CreateLinkedTokenSource(ct);
                docCts.CancelAfter(TimeSpan.FromMinutes(3));

                var result = await DocumentProcessor.ProcessAsync(
                    paperless, llm, doc,
                    settings.ProcessedTagName,
                    settings.ToProcessingOptions(), docCts.Token);
            ...
            {
                // Per-doc 3min timeout — skip, continue with next
                var timeoutMsg = "Zeitüberschreitung (3 min) — Dokument übersprungen.";
                log.LogWarning("Dokument #{Id}: Timeout nach 3 Minuten, wird übersprungen.", doc.Id);
                jobService.MarkFailed(jobId, timeoutMsg);
            }
