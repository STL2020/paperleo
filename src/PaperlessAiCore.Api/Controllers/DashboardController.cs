    ...
    IHostApplicationLifetime appLifetime) : ControllerBase
{
    ...
    public async Task<ActionResult<DashboardDto>> Get(CancellationToken ct)
    {
        ...
        var dto = new DashboardDto
        {
            ...
            CurrentScanDocument = status.CurrentDocument,
        ...
        else
        {
            try
            {
                ...
                // Parallel API calls for speed (4 calls → 1 round-trip time)
                var taskDocs  = paperless.ListDocumentsAsync(new() { ["page_size"] = "1" }, ct);
                var taskTags  = paperless.ListTagsAsync(ct);
                var taskCorr  = paperless.ListCorrespondentsAsync(ct);
                var taskDocTypes = paperless.ListDocumentTypesAsync(ct);

                await Task.WhenAll(taskDocs, taskTags, taskCorr, taskDocTypes);

                var allDocs      = taskDocs.Result;
                var tags         = taskTags.Result;
                var correspondents = taskCorr.Result;
                var docTypes     = taskDocTypes.Result;
                ...
                var processedTag = tags.FirstOrDefault(t =>
                    string.Equals(t.Name, settings.ProcessedTagName, StringComparison.OrdinalIgnoreCase));
                ...
                // Doc type distribution from already-fetched data (no extra API call)
                if (docTypes.Count > 0)
                {
                    dto.DocumentTypeDistribution = docTypes
                        .Take(5)
                        .Select(d => new DocumentTypeCountDto { Type = d.Name, Count = 0 })
                        .ToList();
                }
            }
            catch (Exception ex)
        ...
        // Nur die letzten 500 Einträge für Aggregation — JSONL-Datei wird nicht komplett gelesen
        try
        {
            var activity = await activityLog.GetRecentAsync(500, ct);
        ...
        try
        {
            dto.DotnetVersion = System.Runtime.InteropServices.RuntimeInformation.FrameworkDescription;
            dto.OsDescription = System.Runtime.InteropServices.RuntimeInformation.OSDescription;

            // Uptime des Prozesses
            var uptime = DateTime.UtcNow - System.Diagnostics.Process.GetCurrentProcess().StartTime.ToUniversalTime();
            dto.SystemUptime = uptime.Days > 0
                ? $"{uptime.Days}d {uptime.Hours}h {uptime.Minutes}m"
                : $"{uptime.Hours}h {uptime.Minutes}m {uptime.Seconds}s";

            // Disk-Space: /app/data Verzeichnis (Docker) oder aktueller Pfad
            var dataPath = Directory.Exists("/app/data") ? "/app/data" : AppDomain.CurrentDomain.BaseDirectory;
            var drive = new DriveInfo(Path.GetPathRoot(dataPath) ?? "/");
            dto.DiskTotalBytes = drive.TotalSize;
            dto.DiskUsedBytes  = drive.TotalSize - drive.AvailableFreeSpace;
        }
        catch { /* System-Info ist optional */ }

        // ── KI-Kosten aktueller Monat ─────────────────────────────────────
        try
        {
            var sett = await settingsService.GetAsync(ct);
            dto.MonthTokensUsed    = sett.MonthlyTokenUsed;
            dto.MonthTokenLimit    = sett.MonthlyTokenLimit;
            dto.MonthCostEurEst    = Math.Round(sett.MonthlyTokenUsed / 1_000_000.0 * 0.30, 4);
        }
        catch { }
    ...
    public async Task<ActionResult<ScanNowResult>> ScanNow(CancellationToken ct)
    {
        try
        {
            // ct = HTTP request token (cancelled when browser closes/F5)
            // Scan runs with CancellationToken.None so browser disconnect doesn't abort it
            var settings = await settingsService.GetAsync(ct);
            var count = await scanService.RunScanAsync(settings, CancellationToken.None);
    ...
    {
        // Graceful restart: stop the app, host process (Docker/systemd) will restart it
        Task.Run(async () =>
        {
            await Task.Delay(500); // Give response time to reach client
            appLifetime.StopApplication();
        });
        return Ok(new { success = true, message = "App wird neu gestartet…" });
    }

    [HttpGet("correspondents-full")]
    public async Task<ActionResult<List<CorrespondentDto>>> GetCorrespondentsFull(CancellationToken ct)
    {
        var settings = await settingsService.GetAsync(ct);
        if (string.IsNullOrWhiteSpace(settings.PaperlessUrl)) return Ok(new List<CorrespondentDto>());
        var paperless = new PaperlessClient(httpFactory.CreateClient("paperless"),
            new PaperlessConnectionConfig(settings.PaperlessUrl, settings.PaperlessApiToken));
        var list = await paperless.ListCorrespondentsAsync(ct);
        return Ok(list
            .Select(x => new CorrespondentDto { Id = x.Id, Name = x.Name, DocumentCount = x.DocumentCount })
            .OrderByDescending(x => x.DocumentCount)
            .ThenBy(x => x.Name)
            .ToList());
    }

    [HttpGet("correspondent-docs/{correspondentId}")]
    public async Task<ActionResult<List<CorrespondentDocDto>>> GetCorrespondentDocs(int correspondentId, CancellationToken ct)
    {
        var settings = await settingsService.GetAsync(ct);
        if (string.IsNullOrWhiteSpace(settings.PaperlessUrl)) return Ok(new List<CorrespondentDocDto>());
        var paperless = new PaperlessClient(httpFactory.CreateClient("paperless"),
            new PaperlessConnectionConfig(settings.PaperlessUrl, settings.PaperlessApiToken));

        // Parallel: docs + doc types
        var taskDocs  = paperless.ListDocumentsAsync(
            new() { ["correspondent__id"] = correspondentId.ToString(), ["page_size"] = "100", ["ordering"] = "-created" }, ct);
        var taskTypes = paperless.ListDocumentTypesAsync(ct);
        await Task.WhenAll(taskDocs, taskTypes);

        var typeMap = taskTypes.Result.ToDictionary(t => t.Id, t => t.Name);
        var docs = taskDocs.Result.Results.Select(d => new CorrespondentDocDto
        {
            Id           = d.Id,
            Title        = d.Title,
            Created      = d.Created,
            DocumentType = d.DocumentType.HasValue && typeMap.ContainsKey(d.DocumentType.Value)
                           ? typeMap[d.DocumentType.Value] : null,
            TagIds       = d.Tags,
        }).ToList();
        return Ok(docs);
    }
